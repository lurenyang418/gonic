...
func (c *Controller) ServeGetArtistInfoTwo(r *http.Request) *spec.Response {
	...
	sub.ArtistInfoTwo.Biography = spec.CleanExternalText(artist.Info.Biography)
	sub.ArtistInfoTwo.MusicBrainzID = artist.Info.MusicBrainzID
...
func (c *Controller) ServeGetAlbumInfoTwo(r *http.Request) *spec.Response {
	...
	if err := uuid.Validate(album.TagBrainzID); err == nil {
		sub.AlbumInfo.MusicBrainzID = album.TagBrainzID
	}
...
func (c *Controller) genArtistCoverURL(r *http.Request, artist *db.Artist, size int) string {
	coverURL, _ := url.Parse(middleware.BaseURL(r))
...
func getSimilarSongsFromTrack(c *Controller, id specid.ID, params params.Params, user *db.User, count int) ([]*spec.TrackChild, *spec.Response) {
	...
	err = c.dbc.
		...
		Joins("JOIN track_artists ON track_artists.track_id=tracks.id").
		Joins("JOIN artists ON artists.id=track_artists.artist_id").
		Where("artists.id IN (SELECT artist_id FROM track_artists WHERE track_id=?)", id.Value).
...
func getSimilarSongsFromAlbum(c *Controller, id specid.ID, params params.Params, user *db.User, count int) ([]*spec.TrackChild, *spec.Response) {
	...
	if len(tracks) == 0 {
		return nil, spec.NewError(0, "no similar songs could be matched with collection in database: %v", album.TagTitle)
	}
