...
func New(in string) (ID, error) {
	typ, value, ok := strings.Cut(in, separator)
	if !ok {
	...
	switch IDT(typ) {
	case Playlist:
		return ID{Type: Playlist, StringValue: value}, nil
	}

	val, err := strconv.Atoi(value)
	if err != nil {
		return ID{}, fmt.Errorf("%q: %w", value, ErrNotAnInt)
	}
	...
	default:
		return ID{}, fmt.Errorf("%q: %w", typ, ErrBadPrefix)
	}
}
