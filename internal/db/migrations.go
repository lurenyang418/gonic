...
import (
	...
	"github.com/lurenyang418/gonic/internal/utils"
...
func migratePlaylistsToM3U(tx *gorm.DB, ctx MigrationContext) error {
	...
	locate := func(id specid.ID) string {
		...
		case specid.PodcastEpisode:
			...
			return filepath.Join(ctx.PodcastsPath, pe.Podcast.Title, pe.Filename)
		}
		return ""
	}
...
func migratePlaylistsPaths(tx *gorm.DB, ctx MigrationContext) error {
	...
	for _, p := range podcasts {
		...
		newPath := filepath.Join(ctx.PodcastsPath, p.Title)
		p.RootDir = newPath
		if err := tx.Save(p).Error; err != nil {
